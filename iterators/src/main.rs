...
pub fn remove_odd(a: &[i32]) -> Vec<i32>
{
...
{
    s.split_whitespace().map(|x| x.chars().next().unwrap().to_ascii_uppercase()).collect()
}

fn space_to_underscore(s: &str) -> String
{
    s.chars().map(|x| match x{' ' => '_',_ => x,}).collect()
}
